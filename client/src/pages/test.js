...
const setUserVoted = "http://localhost:5000/api/user";
...
function Test() {
  ...
  const handleVoteToggle = (event) => {
    ...
    // Send API request to update the voted value in the database
    //await fetch(`${setUserVoted}/${currentID}`, {
    fetch(setUserVoted, {
      method: "PUT",
      headers: {
        ...
        <form>
                ...
                {data.map((candidate) => (
                  <div className="test-container">
                    ...
                    <div key={candidate.id} className="candidate-info2">
                      ...
                      <span>localStorage.getItem("currentID")</span>
                    </div>
          ...
          </div>
          {
            <button
              ...
              onClick={(event) => handleVoteToggle(event)}
            >
              Submit
            </button>
          }
